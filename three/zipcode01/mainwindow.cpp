...
void MainWindow::createWidgets()
{
    ...
    minimunZipLabel = new QLabel(tr("Min. Zip:"));
    minimunZipSpinBox = new QSpinBox;
    minimunZipLabel->setBuddy(minimunZipSpinBox);
    minimunZipSpinBox->setRange(MinZipcode, MaxZipcode);
    minimunZipSpinBox->setAlignment(Qt::AlignVCenter | Qt::AlignRight);
...
// 创建两个用于过滤和选择行的 组合框从底层获取数据，，使用代理模型去除重复项
...
{
    delete comboBox->model(); //删除内置的旧模型
    //创建自定义模型UniqueProxyModel，，与底层模型使用同样的列
    UniqueProxyModel *uniqueProxyModel = new UniqueProxyModel(column, this);

    uniqueProxyModel->setSourceModel(model); //设置代理的 源模型为底层模型model
    uniqueProxyModel->sort(column, Qt::AscendingOrder); //代理模型明确对那一列进行唯一值过滤
    comboBox->setModel(uniqueProxyModel); //设置组合框 代理模型
    comboBox->setModelColumn(column);  //设置模型指定 列
}
...
void MainWindow::createLayout()
{
    ...
    groupBoxTopLayout->addWidget(minimunZipLabel);
    groupBoxTopLayout->addWidget(minimunZipSpinBox);
...
void MainWindow::createConnections()
{
    ...
    connect(countyComboBox, SIGNAL(currentIndexChanged(const QString&)),
            this, SLOT(updateUi()));
    connect(stateGroupBox, SIGNAL(toggled(bool)),
            this, SLOT(updateUi()));
    connect(stateComboBox, SIGNAL(currentIndexChanged(const QString&)),
            this, SLOT(updateUi()));
    connect(minimunZipSpinBox, SIGNAL(valueChanged(int)),
            this, SLOT(updateUi()));
...
bool MainWindow::okToClearData()
{
    if(isWindowModified())
        return AQP::okToClearData(&MainWindow::save, this,
                                  tr("Unsaved changed"), tr("Save unsaved changes?"));
    return true;
}
...
void MainWindow::performSelection()
{
    ...
    int minimumZipcode = minimunZipSpinBox->value();
...
{
    proxyModel->setMinimumZipcode(minimunZipSpinBox->value());
